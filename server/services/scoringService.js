const { getUser, getRepos, getEvents, getRepoContents } = require('./githubService');
const computeActivityScore = (events) => {
  ...
  const commitScore = Math.min((commitCount / 50) * 20, 20);
  const activeDays = new Set(
    recentPushes.map((e) => new Date(e.created_at).toDateString())
  );
  const streakScore = Math.min((activeDays.size / 30) * 5, 5);
...
const computeCodeQualityScore = async (repos, username) => {
  ...
  let total = 0;
  let maxPossible = sample.length * 5;
  for (const repo of sample) {
    ...
    if (
      names.some((n) =>
        ['test', 'tests', '__tests__', 'spec', 'specs'].includes(n)
      )
    )
      total += 1;
  }
...
const computeReport = async (username) => {
  ...
  return {
    ...
    heatmapData: buildHeatmapData(events),
